...
const ACCOUNT_EXCHANGE = 'account.exchange';
const ACCOUNT_QUEUE = 'account.queue';
const TRANSACTION_SERVICE_QUEUE = 'transaction.service.queue';
...
async function connect() {
  try {
    ...
    logger.info(`Exchange ${ACCOUNT_EXCHANGE} asserted`);

    // Assert account queue
    await channel.assertQueue(ACCOUNT_QUEUE, { durable: true });
    await channel.bindQueue(ACCOUNT_QUEUE, ACCOUNT_EXCHANGE, 'account.#');
    logger.info(`Queue ${ACCOUNT_QUEUE} asserted and bound to exchange`);

    // Assert transaction service queue
    await channel.assertQueue(TRANSACTION_SERVICE_QUEUE, { durable: true });
    await channel.bindQueue(TRANSACTION_SERVICE_QUEUE, ACCOUNT_EXCHANGE, 'transaction.#');
    logger.info(`Queue ${TRANSACTION_SERVICE_QUEUE} asserted and bound to exchange`);

    // Assert notification service queue
    await channel.assertQueue(NOTIFICATION_SERVICE_QUEUE, { durable: true });
    await channel.bindQueue(NOTIFICATION_SERVICE_QUEUE, ACCOUNT_EXCHANGE, 'notification.#');
